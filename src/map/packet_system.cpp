...
PChar->pushPacket(new CInventoryItemPacket(NULL, FromLocationID, FromSlotID));	// ??????? ??????? ?? FormLocationID
PChar->pushPacket(new CInventoryItemPacket(PItem, ToLocationID, NewSlotID));	// ????????? ??????? ? ToLocationID
}
else // ? ?????? ?????? ???????? ??????????? ????????
{
PChar->getStorage(ToLocationID)->InsertItem(NULL, NewSlotID); // ??????? ???????
PChar->getStorage(FromLocationID)->InsertItem(PItem, FromSlotID); // ?????????? ??????? (??? ?????????? Location ? Slot ????????)
}
}
else
{
...
uint8 town	= RBUFB(data,(0x16)); // ???????????? ??? ?????? ?? mog house
uint8 zone	= RBUFB(data,(0x17)); // ???????????? ??? ?????? ?? mog house
// ??????? ????? ?????? ? ?????????????? ??????? zoneline
//
// ?????????? ??? zoneline ??????,
// ???? ?? ???????? ??????? ???????
...
if (zoneutils::GetZone(PZoneLine->m_toZone)->GetIP() == 0) // ????????????? ????????? ?? 180° ? ?????????? ????, ?????? ??????
{
...
case 3: // ????????? merit
{
if (PChar->getZone() == 0) // ??? ???????? ?????????????? ?????? ? moghouse
{
MERIT_TYPE merit = (MERIT_TYPE)(RBUFW(data,(0x06)) << 1);
if (PChar->PMeritPoints->IsMeritExist(merit)) // ????????? ?????????? ?????????? id
{
switch (operation)
{
...
if (PItemLinkshell != NULL && PItemLinkshell->isType(ITEM_LINKSHELL))
{
if (PItemLinkshell->getID() == 512) // ???????? ????? linkshell
{
...
/************************************************************************
* *
* ????????? ?????? nameFlags: party, away, autogroup, *
* ????? ? help desk ? ?.?. *
* *
************************************************************************/
...
PChar->resetPetZoningInfo();
charutils::RemoveAllEquipment(PChar); // TODO: ???????????, ?????
PChar->SetMJob(mjob);
