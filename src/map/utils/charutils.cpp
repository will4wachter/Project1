...
// ?????? ???????? HP ?? ?????????????? ?????????
int32 subLevelOver10 = dsp_cap(slvl - 10, 0, 20);	// +1HP ?? ?????? ??????? ????? 10 (/2)
int32 subLevelOver30 = (slvl < 30 ? 0 : slvl - 30);	// +1HP ?? ?????? ??????? ????? 30
...
{
raceStat = (grade::GetMPScale(grade,0) + grade::GetMPScale(grade,scaleTo60Column) * (slvl - 1)) / 2; // ??? ????? ??????
}
}else{
//?????? ??????????? ???????? ??????
...
PChar->health.maxmp = (int16)(raceStat + jobStat + sJobStat + MeritBonus); // ????????? ??????? MP
...
{
if (Sql_GetIntData(SqlHandle,10) != 0) // ?????? ??????, ??? ??????? ????????????? ??????????
{
...
PItemContainer->InsertItem(NULL, NewSlotID); // ???????? ??? ????????? ??????????
}
}
...
{
PChar->PRecastContainer->Del(RECAST_ITEM, PItem->getSlotID()); // ??? ?????? ???????? ? ???????? ??????? ?????? ? ??? ?? RecastList
}
